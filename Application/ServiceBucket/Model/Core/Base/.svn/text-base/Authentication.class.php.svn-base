...
{	
	...
	{	
		$sql = "SELECT 
					{$profileName}_ID, {$profileName}_USERNAME, {$profileName}_PASSWORD
				FROM
					{$profileName}
				WHERE 
					{$profileName}_USERNAME = '{$userName}' AND {$profileName}_PASSWORD = '" . md5($password) . "'";
		...
		if ( $database->countRows($userRes) > 0 ) 
		{
		    ...
		    $session["{$profileName}_USERNAME"] = $userName;
